function createPedestrianCount(viewer) {
	...
	return function(json) {
		...
		var extentSlice = new Cesium.ExtentPrimitive({
			extent : Cesium.Extent.fromDegrees(e.west - delta, e.south - delta,
					e.east + delta, e.north + delta),
			show : false,
			asynchronous : false
		});
		...
		function animateExtentSlice(id) {
			...
			scene.getAnimations().addProperty(extentSlice, 'height',
					extentSlice.height, id.top, {
						duration : 600,
						onComplete : function() {
		...
		for ( var k = 0; k < cameras.length; ++k) {
			...
			var center = ellipsoid.cartographicToCartesian(Cesium.Cartographic
					.fromDegrees(camera.longitude, camera.latitude));
			 ...
			 */
			/*
			 * 'Early Morning', 'Morning RH ', 'Late Morning', 'Lunch', 'Late
			 * Afternoon', 'Evening RH', 'Evening', 'Late Night'
			 */
			];
